 ...
 * №1521. War Games 2
 *
 * Link: https://acm.timus.ru/problem.aspx?space=1&num=1521
 *
 * Idea: Build Red-Black BST, where all operations are O(logN),
 * then we may just iterate over it.
 *
 * This solution gets AC with 0.671s of time and 12 940KB of memory
 */
import java.util.*
...
fun main() {
    val soldiers = RedBlackBST<Int, Int>()
    ...
    for (i in 0..<n) {
        ...
        val next_soldier = soldiers.select(current_index)
        print("${next_soldier + 1} ")
        soldiers.delete(next_soldier)
        current_size--
    }
}